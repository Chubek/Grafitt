...
bfs_forest_cover(
    ...
    for (const auto& root : verts) {
        ...
        while (!q.empty()) {
            const V cur = q.front();
            q.pop();
            piece.vertices.push_back(cur);
            out.vertex_to_piece[cur] = pid;
            ...
            if (options.max_piece_vertices > 0 && piece.vertices.size() >= options.max_piece_vertices) {
                while (!q.empty()) q.pop();
                break;
            }
        }
...
[[nodiscard]] inline saturation_summary saturate(
    ...
    saturation_trace dummy_trace;
    return saturate(eg, rules, dummy_trace, options, rule_schedule_policy::stable);
}
